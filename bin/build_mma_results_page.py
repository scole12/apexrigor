#!/usr/bin/env python3
"""MMA results page: NFL/NCAAF chrome; Overall = fused APEX TOTAL RECORD."""
from __future__ import annotations

from pathlib import Path

from _mma_public import ROOT, write
from apply_cloudflare_web_analytics import BEACON_BLOCK
from apply_vercel_web_analytics import ANALYTICS_BLOCK

CACHE = "mma-total-apex-003"
  ...
  async function load() {
    ...
    for (const ev of archive.events || []) {
      ...
      for (const r of ev.latest_results || []) {
        ...
        rows.push({
          date, event: eventName, matchup: r.matchup || r.bout_name || r.selection || "—",
          market: "WINNER", pick: r.selection || "—", tier: r.rating_tier || r.tier || "—", result,
        });
      }
    }
    ...
    html += `<div class="banner" data-apex-season-record="${esc(fmtRecord(ow,ol,op))}" data-apex-season-win-rate="${esc(owr)}">
    ...
    for (const [date, selected] of sortedDays) {
      ...
      html += table(["Bout","Matchup","Market","Pick","Tier","Result"], selected.map((row, idx) => [`F${String(idx+1).padStart(2,"0")}`, row.matchup, row.market, row.pick, row.tier, row.result]));
    }
...
def main() -> None:
    ...
    (out / "render.js").write_text(RENDER, encoding="utf-8")
    write(out / "index.html", HTML)
    print(f"MMA_RESULTS_PATH={out / 'index.html'}")
