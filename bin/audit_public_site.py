...
def main() -> int:
    ...
    if nfl_route_established:
        ...
        if nfl_today:
            ...
            if issuance is True:
                issued = True
            elif isinstance(issuance, dict) and str(issuance.get("status", "")).upper() in {"ISSUED", "PUBLIC", "LIVE"}:
                issued = True
            sci = str(nfl_today.get("scientific_release_state") or "").upper()
            if issued:
                if not positions:
                    errors.append("issued NFL payload has zero public positions")
                pos_count = nfl_today.get("position_count")
                if pos_count is not None and int(pos_count) != len(positions):
                    errors.append("NFL position_count does not reconcile to positions length")
                if isinstance(issuance, dict) and issuance.get("position_count") is not None:
                    if int(issuance["position_count"]) != len(positions):
                        errors.append("NFL public_issuance.position_count does not reconcile")
            else:
                if positions:
        ...
        if nfl_state:
            # Scott 2026-09-09: no SCIENCE_BLOCKED invent-gates; READY or explicit engine map OK
            ...
            if sci_state not in {"READY", "ISSUED", "LIVE"} and isinstance(science, dict):
                blocked = {k: v for k, v in science.items() if "BLOCK" in str(v).upper()}
                if blocked and nfl_today.get("positions"):
            ...
            if not hydration and nfl_today:
                # parity may live on today payload
                th = nfl_today.get("hydration") or {}
                if th and th.get("source_to_target_parity") not in (None, "PASS"):
                    errors.append("NFL public state is not backed by source-to-target parity")
        # results may be non-zero once issued; no science-blocked zero-force
