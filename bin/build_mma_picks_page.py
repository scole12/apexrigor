...
from _mma_forecast_contract import validated_positions
...
function issuedPositions(d){
 ...
 return rows.filter(p=>p.market==="WINNER"||p.market==="METHOD");
}
...
fetch("/data/mma_today.json",{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error("HTTP "+r.status);return r.json()}).then(d=>{
 ...
 const meta=dateLabel(e.event_date)+" · "+card.length+" SCHEDULED BOUTS · "+positions.length+" POSITIONS"
  +(positions.length?"":" · UNISSUED · AWAITING T-2")+(d.active_model?" · MODEL "+d.active_model:"");
 const html=card.map(({b,positions:issued},i)=>{
  ...
  if(!issued.length){
   panels='<section class="market-panel" data-position-state="UNISSUED"><div class="market-label">STATUS</div><div class="market-panel-head"><span class="pick-headline">UNISSUED — AWAITING T-2</span><span class="tier-badge tier-badge--moderate">SCHEDULED</span></div><div class="rationale-copy"><p>Listed from the official card. No sealed FanDuel-issued APEX positions yet.</p></div></section>';
  }
...
def main():
    ...
    validated_positions(json.loads((ROOT / "data/mma_today.json").read_text()))
